...
{
public:
    WatcherSlice(std::vector<HANDLE>::iterator &begin, std::vector<HANDLE>::iterator &end, std::function<bool(const Path &)> &&notif,
...
WatcherSlice::WatcherSlice(std::vector<HANDLE>::iterator &begin, std::vector<HANDLE>::iterator &end,
                           std::function<bool(const Path &)> &&notif, const std::map<HANDLE, Path> &paths)
    : changes(begin, end)
    , stopped(false)
...
void WatcherData::updatePaths()
{
    ...
    for (const Path &path : paths) {
        ...
        // printf("hello %s\n", winPath.c_str());
        const HANDLE h = FindFirstChangeNotification(winPath.c_str(), TRUE, FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_LAST_WRITE);
#else
        const HANDLE h = FindFirstChangeNotification(path.c_str(), TRUE, FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_LAST_WRITE);
#endif
    ...
    for (;;) {
        if (hasChanged()) {
            ...
            while (where != end) {
                ...
                if (to > end)
                    to = end;
                slices.push_back(std::unique_ptr<WatcherSlice>(
                    new WatcherSlice(where, to, std::bind(&WatcherData::updated, this, std::placeholders::_1), handleToPath)));
                where = to;
            }
        }
        ...
        if (!changedPaths.empty()) {
            for (const Path &p : changedPaths) {
                ...
                if (!removed.empty())
                    EventLoop::mainEventLoop()->callLaterMove(std::bind(&FileSystemWatcher::pathsRemoved, watcher, std::placeholders::_1),
                                                              std::move(removed));
                if (!data.added.empty())
                    EventLoop::mainEventLoop()->callLaterMove(std::bind(&FileSystemWatcher::pathsAdded, watcher, std::placeholders::_1),
                                                              std::move(data.added));
                if (!data.changed.empty())
                    EventLoop::mainEventLoop()->callLaterMove(std::bind(&FileSystemWatcher::pathsModified, watcher, std::placeholders::_1),
