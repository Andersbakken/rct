...
public:
    ...
    void setErrorHandler(std::function<void(const std::shared_ptr<SocketClient> &, Message::MessageError &&)> handler)
    {
        mErrorHandler = handler;
    }

    std::function<void(const std::shared_ptr<SocketClient> &, Message::MessageError &&)> errorHandler() const
    {
    ...
    {
        return mSocketClient && mSocketClient->isConnected();
    }

    Signal<std::function<void(std::shared_ptr<Connection>)>> &sendFinished()
    {
        return mSendFinished;
    }

    Signal<std::function<void(std::shared_ptr<Connection>)>> &connected()
    {
        return mConnected;
    }

    Signal<std::function<void(std::shared_ptr<Connection>)>> &disconnected()
    {
        return mDisconnected;
    }

    Signal<std::function<void(std::shared_ptr<Connection>)>> &error()
    {
        return mError;
    }

    Signal<std::function<void(std::shared_ptr<Connection>, int)>> &finished()
    {
        return mFinished;
    }

    Signal<std::function<void(std::shared_ptr<Connection>, const Message *)>> &aboutToSend()
    {
        return mAboutToSend;
    }

    Signal<std::function<void(std::shared_ptr<Message>, std::shared_ptr<Connection>)>> &newMessage()
    {
        return mNewMessage;
    }

    std::shared_ptr<SocketClient> client() const
    {
...
private:
    ...
    void onClientConnected(const std::shared_ptr<SocketClient> &)
    {
        mIsConnected = true;
        mConnected(shared_from_this());
    }

    void onClientDisconnected(const std::shared_ptr<SocketClient> &)
    {
        mIsConnected = false;
