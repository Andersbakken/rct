...
void FileSystemWatcher::shutdown()
{
    ...
    for (Map<Path, int>::const_iterator it = mWatchedByPath.begin(); it != mWatchedByPath.end(); ++it) {
        ...
        if (::kevent(mFd, &change, 1, 0, 0, &nullts) == -1) {
            // bad stuff
            error("FileSystemWatcher::~FileSystemWatcher() kevent failed for '%s' (%d) %s", it->first.c_str(), errno, Rct::strerror().c_str());
        }
        ::close(it->second);
    }
    close(mFd);
...
bool FileSystemWatcher::watch(const Path &p)
{
    ...
    if (ret != -1) {
        ...
        if (::kevent(mFd, &change, 1, 0, 0, &nullts) == -1) {
            ...
            error("FileSystemWatcher::watch() kevent failed for '%s' (%d) %s", path.c_str(), errno, Rct::strerror().c_str());
            ::close(ret);
            return false;
        }
    }
...
bool FileSystemWatcher::unwatch(const Path &p)
{
    ...
    if (mWatchedByPath.remove(path, &wd)) {
        ...
        if (::kevent(mFd, &change, 1, 0, 0, &nullts) == -1) {
            // bad stuff
            error("FileSystemWatcher::unwatch() kevent failed for '%s' (%d) %s", path.c_str(), errno, Rct::strerror().c_str());
        }
        ::close(wd);
    ...
    FSUserData data;
    {
        ...
        for (;;) {
            ...
            if (ret == 0) {
                ...
                error("FileSystemWatcher::notifyReadyRead() kevent failed (%d) %s", errno, Rct::strerror().c_str());
                break;
            }
            ...
            for (int i = 0; i < ret; ++i) {
                ...
                if (event.flags & EV_ERROR) {
                    error("FileSystemWatcher::notifyReadyRead() kevent element failed for '%s' (%ld) %s", p.c_str(), event.data, Rct::strerror(event.data).c_str());
                    continue;
                }
                if (p.empty()) {
                ...
                if (event.fflags & (NOTE_DELETE | NOTE_REVOKE | NOTE_RENAME)) {
                    ...
                    } signals[] = { { mModified, data.modified }, { mAdded, data.added } };
