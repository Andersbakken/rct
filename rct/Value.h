...
public:
    ...
    inline Value(unsigned long i)
        : mType(Type_Integer)
    {
        mData.llong = i;
    }

    inline Value(long long i)
        : mType(Type_Integer)
    {
        mData.llong = i;
    }

    inline Value(unsigned long long i)
        : mType(Type_Integer)
    {
        mData.ullong = i;
    }

    inline Value(float d)
        : mType(Type_Double)
    {
        mData.dbl = d;
    }
    ...
    inline Value(const std::shared_ptr<Custom> &custom)
        : mType(Type_Custom)
    {
        new (mData.customBuf) std::shared_ptr<Custom>(custom);
    }

    inline Value(const String &string)
        : mType(Type_String)
    {
        new (mData.stringBuf) String(string);
    }

    inline Value(const Date &date)
        : mType(Type_Date)
    {
        mData.llong = date.time();
    }
    ...
    inline Value(const char *str, int len = -1)
        : mType(Type_String)
    {
        if (len == -1)
            len = strlen(str);
    ...
    inline Value(const Map<String, Value> &map)
        : mType(Type_Map)
    {
        new (mData.mapBuf) Map<String, Value>(map);
    }

    template <typename T>
    inline Value(const List<T> &list)
        : mType(Type_List)
    {
    ...
    inline Value(const List<Value> &list)
        : mType(Type_List)
    {
        new (mData.listBuf) List<Value>(list);
    }
    ...
    template <typename T>
    inline T value(int idx, const T &defaultValue = T(), bool *ok = nullptr) const;
    inline Value value(const String &key, const Value &defaultValue = Value()) const;
    template <typename T>
    inline T value(const String &name, const T &defaultValue = T(), bool *ok = nullptr) const;

    template <typename T>
    inline T convert(bool *ok = nullptr) const
    {
        invalidType(T());
        if (ok)
            *ok = false;
    ...
    static Value fromJSON(const String &json, bool *ok = nullptr)
    {
        return fromJSON(json.c_str(), ok);
    }
    ...
    public:
        ...
        {
            String ret;
            format(value, [&ret](const char *str, size_t len)
                   {
                       ret.append(str, len);
                   });
            return ret;
...
private:
    ...
    {
        return pun<const String>();
    }

    Map<String, Value> *mapPtr()
    {
        return pun<Map<String, Value>>();
    }

    const Map<String, Value> *mapPtr() const
    {
        return pun<const Map<String, Value>>();
    }

    List<Value> *listPtr()
    {
        return pun<List<Value>>();
    }

    const List<Value> *listPtr() const
    {
        return pun<const List<Value>>();
    }

    std::shared_ptr<Custom> *customPtr()
    {
        return pun<std::shared_ptr<Custom>>();
    }

    const std::shared_ptr<Custom> *customPtr() const
    {
        return pun<const std::shared_ptr<Custom>>();
    }
...
template <>
inline int Value::convert<int>(bool *ok) const
{
    if (ok)
        *ok = true;
    switch (mType) {
        case Type_Date:
            return static_cast<int>(mData.llong);
        case Type_Integer:
            return mData.integer;
        case Type_Double:
            return static_cast<int>(round(mData.dbl));
        case Type_Boolean:
...
template <>
inline long long Value::convert<long long>(bool *ok) const
{
    if (ok)
        *ok = true;
    switch (mType) {
        ...
        case Type_Integer:
            return mData.llong;
        case Type_Double:
            return static_cast<long long>(round(mData.dbl));
        case Type_Boolean:
            return mData.boolean;
        case Type_String: {
            char *endPtr;
...
template <>
inline unsigned long long Value::convert<unsigned long long>(bool *ok) const
{
    if (ok)
        *ok = true;
    switch (mType) {
        ...
        case Type_Integer:
            return mData.ullong;
        case Type_Double:
            return static_cast<unsigned long long>(round(mData.dbl));
        case Type_Boolean:
            return mData.boolean;
        case Type_String: {
            char *endPtr;
...
template <>
inline std::shared_ptr<Value::Custom> Value::convert<std::shared_ptr<Value::Custom>>(bool *ok) const
{
    if (ok)
        *ok = true;
    switch (mType) {
...
template <>
inline bool Value::convert<bool>(bool *ok) const
{
    if (ok)
        *ok = true;
...
template <>
inline double Value::convert<double>(bool *ok) const
{
    if (ok)
        *ok = true;
    ...
    switch (mType) {
        case Type_Date:
            return static_cast<double>(mData.llong);
        case Type_Integer:
            return mData.integer;
        case Type_Double:
            return mData.dbl;
...
template <>
inline String Value::convert<String>(bool *ok) const
{
    if (ok)
        *ok = true;
    ...
    switch (mType) {
        case Type_Date:
            return String::number(mData.llong);
        case Type_Integer:
            return String::number(mData.integer);
        case Type_Double:
            return String::number(mData.dbl);
        case Type_Boolean:
            return mData.boolean ? "true" : "false";
        case Type_String:
            return *stringPtr();
        case Type_Invalid:
            break;
...
template <>
inline List<Value> Value::convert<List<Value>>(bool *ok) const
{
    if (ok)
        *ok = true;
...
template <>
inline Map<String, Value> Value::convert<Map<String, Value>>(bool *ok) const
{
    if (ok)
        *ok = true;
...
inline Value Value::convert(Type type, bool *ok) const
{
    switch (type) {
        case Type_Date:
            return convert<long long>(ok);
        case Type_Integer:
            return convert<int>(ok);
        case Type_Double:
            return convert<double>(ok);
        case Type_Boolean:
            return convert<bool>(ok);
        case Type_String:
            return convert<String>(ok);
        case Type_Invalid:
            if (ok)
                *ok = true;
            return Value();
        ...
        case Type_Custom:
            return convert<std::shared_ptr<Custom>>(ok);
        case Type_List:
            return convert<List<Value>>(ok);
        case Type_Map:
            return convert<Map<String, Value>>(ok);
    }
    if (ok)
        *ok = false;
    return Value();
...
{
    return convert<int>(nullptr);
}

inline long long Value::toLongLong() const
{
    return convert<long long>(nullptr);
}

inline unsigned long long Value::toULongLong() const
{
    return convert<unsigned long long>(nullptr);
}

inline double Value::toDouble() const
{
    return convert<double>(nullptr);
}

inline Date Value::toDate(Date::Mode mode) const
{
    return Date(convert<long long>(nullptr), mode);
}

inline String Value::toString() const
{
    return convert<String>(nullptr);
}

inline std::shared_ptr<Value::Custom> Value::toCustom() const
{
    return convert<std::shared_ptr<Custom>>(nullptr);
}

inline Map<String, Value> Value::toMap() const
{
    return convert<Map<String, Value>>(nullptr);
}

inline List<Value> Value::toList() const
{
    return convert<List<Value>>(nullptr);
}
...
inline Log operator<<(Log log, const Value &value)
{
    ...
    log << String::format<128>("Value(%s: %s)", Value::typeToString(value.type()), str.c_str());
    return log;
}
...
inline Serializer &operator<<(Serializer &serializer, const Value &value)
{
    ...
    switch (value.type()) {
        case Value::Type_Date:
            serializer << value.toLongLong();
            break;
        case Value::Type_Integer:
            serializer << value.toInteger();
            break;
        case Value::Type_Double:
            serializer << value.toDouble();
            break;
        case Value::Type_Boolean:
        ...
        case Value::Type_String:
            ...
            break;
        case Value::Type_Map:
            serializer << value.toMap();
            break;
        case Value::Type_List:
            serializer << value.toList();
            break;
        case Value::Type_Custom:
            error() << "Trying to serialize pointer";
            break;
        case Value::Type_Invalid:
            break;
        case Value::Type_Undefined:
...
inline Deserializer &operator>>(Deserializer &deserializer, Value &value)
{
    ...
    switch (type) {
        ...
        case Value::Type_Custom:
            value.clear();
            error() << "Trying to deserialize pointer";
            break;
        case Value::Type_Invalid:
            value.clear();
            break;
