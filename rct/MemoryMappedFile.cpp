...
MemoryMappedFile::MemoryMappedFile()
    ...
    , mhFileMapping(INVALID_HANDLE_VALUE)
    , mFileSize(0)
#else
    mFd(-1)
    , mFileSize(0)
...
MemoryMappedFile::MemoryMappedFile(const Path &f_file, AccessType f_access, LockType f_lock)
    : mpMapped(nullptr)
    , mAccessType(NO_ACCESS)
    ,
#ifdef _WIN32
...
bool MemoryMappedFile::open(const Path &f_filename, AccessType f_access, LockType f_lock)
{
    if (isOpen())
        close();
    ...
    const DWORD access        = (f_access == READ_ONLY) ? GENERIC_READ : (GENERIC_READ | GENERIC_WRITE);
    const DWORD share         = (f_lock == DO_LOCK) ? 0 : (FILE_SHARE_READ | FILE_SHARE_WRITE);
    const DWORD protect       = (f_access == READ_ONLY) ? PAGE_READONLY : PAGE_READWRITE;
    const DWORD desiredAccess = (f_access == READ_ONLY) ? FILE_MAP_READ : FILE_MAP_WRITE; // FILE_MAP_WRITE includes read access

    // first, we need to open the file:
    mhFile = CreateFileW(Utf8To16(f_filename.c_str()), access, share, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    mpMapped = MapViewOfFile(mhFileMapping, desiredAccess, 0, 0, // offset high and low
                             0);                                 // size
    ...
    const int openFlags = (f_access == READ_ONLY) ? O_RDONLY : O_RDWR;
    const int protFlags = (f_access == READ_ONLY) ? PROT_READ : (PROT_READ | PROT_WRITE);
    ...
    struct stat st;
    if (fstat(mFd, &st) != 0) {
        error() << "Could not stat file " << f_filename << ". errno=" << errno;
        close();
        return false;
    }
    ...
    if (mpMapped == MAP_FAILED) {
        mpMapped = nullptr;
        close();
        error() << "Could not map file " << f_filename << ". errno=" << errno;
        return false;
    }
...
void MemoryMappedFile::close()
{
    ...
    if (mpMapped && !UnmapViewOfFile(mpMapped)) {
        error() << "Could not UnmapViewOfFile(). GetLastError()=" << GetLastError();
    }
    ...
    if (mpMapped != nullptr && munmap(mpMapped, mFileSize) != 0) {
        error() << "Could not unmap " << mFilename << ". errno=" << errno;
    }
    ...
    if (mFd != -1) {
        ...
        if (ret == -1) {
            error() << "Could not close file " << mFilename << ". errno=" << errno;
        }
