...
public:
    ...
    static void
    registerListOption(const char *name, const String &description, const char shortOpt = '\0', const List<T> &defaultValue = List<T>(),
    ...
    template <typename T>
    static T value(const char *name, const T &defaultValue, bool *ok = nullptr)
    {
        const OptionBase *opt = findOption(name);
    ...
    template <typename T>
    static T value(const char *name, bool *ok = nullptr)
    {
        const OptionBase *opt = findOption(name);
        if (opt) {
            if (opt->value.isNull()) {
    ...
    static void setAllowsFreeArguments(bool on)
    {
        sAllowsFreeArgs = on;
    }

    static bool allowsFreeArguments()
    {
        return sAllowsFreeArgs;
    }

    static List<Value> freeArgs()
    {
        return sFreeArgs;
    }
