...
public:
    ...
    LogLevel operator++(int)
    {
        return LogLevel(mValue++);
    }
...
public:
    ...
    {
        log(Flags<LogFlag>(DefaultFlags), msg.c_str(), msg.length());
    }
...
bool initLogging(const char *ident, Flags<LogFlag> flags = LogStderr, LogLevel logLevel = LogLevel::Error, const Path &logFile = Path(),
...
public:
...
#ifndef __i386__
    Log operator<<(long number)
    {
        return addStringStream(number);
    }
#endif
    ...
    Log operator<<(uint64_t number)
    {
        return addStringStream(number);
    }

    Log operator<<(int64_t number)
    {
        return addStringStream(number);
    }
#endif
...
#endif
    Log operator<<(unsigned long long number)
    {
        return addStringStream(number);
    }

    Log operator<<(long long number)
    {
        return addStringStream(number);
    }
#ifdef _WIN32
    ...
    /// just like uint32_t.
    Log operator<<(unsigned long number)
    {
        return addStringStream(number);
    }
#endif
    Log operator<<(uint32_t number)
    {
        return addStringStream(number);
    }

    Log operator<<(int32_t number)
    {
        return addStringStream(number);
    }

    Log operator<<(uint16_t number)
    {
        return addStringStream(number);
    }

    Log operator<<(int16_t number)
    {
        return addStringStream(number);
    }

    Log operator<<(uint8_t number)
    {
        return addStringStream<uint16_t>(number);
    }

    Log operator<<(int8_t number)
    {
        return addStringStream(number);
    }
    ...
    Log operator<<(long double number)
    {
        return addStringStream(number);
    }
