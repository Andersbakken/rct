...
Process::~Process()
{
    ...
    if (mthStdout.joinable())
        mthStdout.join();
    if (mthStderr.joinable())
        mthStderr.join();
    if (mthManageTimeout.joinable())
        mthManageTimeout.join();
}
...
Process::ExecState Process::exec(const Path &f_cmd, const List<String> &f_args, int f_timeout_ms, unsigned int f_flags)
{
    // call exec() with default-constructed f_environ
    return exec(f_cmd, f_args, List<String>(), f_timeout_ms, f_flags);
}

Process::ExecState Process::exec(const Path &f_cmd, const List<String> &f_args, const List<String> &f_environ, int f_timeout_ms,
...
bool Process::start(const Path &f_cmd, const List<String> &f_args, const List<String> &f_environ)
{
    mMode = Async;
    return startInternal(f_cmd, f_args, f_environ) == Done;
}

Process::ExecState Process::startInternal(const Path &f_cmd, const List<String> &f_args, const List<String> &f_environ, int f_timeout_ms,
                                          unsigned int f_flags)
{
    ...
    if (!CreatePipe(&mStdIn[READ_END], &mStdIn[WRITE_END], &saAttr, 0) || !CreatePipe(&mStdOut[READ_END], &mStdOut[WRITE_END], &saAttr, 0) || !CreatePipe(&mStdErr[READ_END], &mStdErr[WRITE_END], &saAttr, 0)) {
        error() << "Error creating pipes";
        return Error;
    }

    // the child is not supposed to gain access to the pipes' parent end
    if (!SetHandleInformation(mStdIn[WRITE_END], HANDLE_FLAG_INHERIT, 0) || !SetHandleInformation(mStdOut[READ_END], HANDLE_FLAG_INHERIT, 0) || !SetHandleInformation(mStdErr[READ_END], HANDLE_FLAG_INHERIT, 0)) {
...
{
    std::unique_lock<std::mutex> lock(mMutex);
    if (mProcessFinished_cond.wait_for(lock, std::chrono::milliseconds(timeout_ms), [this]()
                                       {
                                           return mProcessTimeoutStatus == FINISHED_ON_ITS_OWN;
                                       })) {
    ...
    if (!WriteFile(mStdIn[WRITE_END], f_data.data(), f_data.size(), NULL, NULL)) {
        ...
        switch (errorCode) {
            case ERROR_BROKEN_PIPE:
                ...
                break;
            default:
                error() << "Error writing to child process' stdin. GetLastError()=" << errorCode;
        }
    }
}
...
void Process::kill(int f_signal)
{
    ...
    if (mProcess.hProcess == INVALID_HANDLE_VALUE)
        return;
    if (mProcessTimeoutStatus == KILLED || mProcessTimeoutStatus == FINISHED_ON_ITS_OWN)
        return;
