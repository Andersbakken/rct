...
public:
    ...
    static std::shared_ptr<EventLoop> mainEventLoop()
    {
        std::lock_guard<std::mutex> locker(mMainMutex);
        return sMainLoop.lock();
    }
    ...
    {
        return EventLoop::mainEventLoop() && std::this_thread::get_id() == EventLoop::mainEventLoop()->threadId;
    }
...
private:
    ...
    struct TimerDataSet
    {
        bool operator()(TimerData *a, TimerData *b) const
        {
    ...
    struct TimerDataHash
    {
        ...
        size_t operator()(TimerData *a) const
        {
            return a->id;
        }
