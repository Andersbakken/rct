...
public:
    ...
    Flags(const Flags<T> &other)
    {
        mValue = other.mValue;
    }

    Flags(Flags &&other) noexcept
        : mValue(other.mValue)
    {
        other.mValue = 0;
    }
    ...
    static Flags<T> construct(int64_t value)
    {
        Flags<T> ret;
        ret.mValue = value;
        return ret;
    ...
    Flags<T> &operator=(Flags<T> &&other)
    {
        mValue       = other.mValue;
        other.mValue = 0;
        return *this;
    ...
    Flags<T> &operator=(const Flags<T> &other)
    {
        mValue = other.mValue;
        return *this;
    }

    Flags<T> &operator=(T t)
    {
        mValue = t;
        return *this;
    }
...
#define ADD_OPERATOR(OP)                                                                                                      \
    ...
    {
        return mValue ? &safeBool : nullptr;
    }
    ...
    Flags<T> test(Flags<T> flags) const
    {
        return construct(mValue & flags);
    }

    template <typename F>
    void set(F flag, bool on = true)
    {
        if (on) {
            *this |= flag;
        } else {
    ...
    bool operator==(Flags<T> other) const
    {
    ...
    {
        return mValue == static_cast<int64_t>(other);
    }
...
#define RCT_FLAGS(T)                     \
    ...
    inline Flags<T> operator~(T t)       \
    {                                    \
        return Flags<T>::construct(~+t); \
    }
...
#define RCT_SUBFLAGS(ORIGINAL, SUB)              \
    ...
    inline Flags<ORIGINAL> operator~(SUB t)      \
    {                                            \
        return Flags<ORIGINAL>::construct(~+t);  \
    }
...
public:
    ...
    static Flags<T> construct(uint64_t value)
    {
        Flags<T> ret;
        ret.mValue = value;
        return ret;
    ...
    unsigned int test(unsigned long long flag) const
    {
        return mValue & flag;
    }
